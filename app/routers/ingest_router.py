...
from app.schemas.ingest import IngestStatusResponse, ReindexResponse
...
# ============================================================
# INICIO: Re-indexacion de documentos desde Supabase (V-06)
# ============================================================
...
@router.post(
    ...
    summary="Re-indexar CFDIs de tu org desde Supabase a ChromaDB",
    description=(
        "Toma todos los CFDIs extraidos (tabla extracciones) de tu organizacion "
        "y los re-ingesta en la coleccion organizacional de ChromaDB. "
        "Util util cuando se borra el volumen de ChromaDB (ej: docker compose down). "
        "No re-ejecuta OCR ni Mistral -- usa los metadatos ya almacenados en Supabase. "
        "Opcional: ?reset=true para limpiar la coleccion antes de re-indexar. "
        "**Requiere token JWT.**"
    ),
)
def reindex_org(
    ...
    from app.services.org_ingestion_service import reindexar_org_desde_supabase
    ...
    try:
        resultado = reindexar_org_desde_supabase(
            id_organizacion=usuario.id_organizacion,
            id_usuario=usuario.id,
            reset=reset,
        )
        status_str = "completado" if resultado["fallidos"] == 0 else "completado_con_errores"
        return ReindexResponse(
            status=status_str,
            mensaje=(
                f"Re-indexacion {status_str}: {resultado['exitosos']} exitosos, "
                f"{resultado['fallidos']} fallidos de {resultado['intentados']}"
            ),
            **resultado,
        )
    except Exception as exc:
...
# ============================================================
# FIN: Re-indexacion de documentos desde Supabase
# ============================================================
