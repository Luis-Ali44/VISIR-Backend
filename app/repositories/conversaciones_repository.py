...
def crear_sesion(id_usuario: str, id_organizacion: str) -> str | None:
    payload = {
    ...
    try:
        response = supabase.table("sesiones_conversacion").insert(payload).execute()
        data = response.data
...
def listar_sesiones(id_usuario: str, id_organizacion: str) -> list[dict[str, Any]]:
    try:
        response = (
            supabase.table("sesiones_conversacion")
...
def obtener_sesion(sesion_id: str, id_organizacion: str) -> dict[str, Any] | None:
    try:
        response = (
            supabase.table("sesiones_conversacion")
            .select("*")
...
def obtener_turnos_por_sesion(sesion_id: str, id_organizacion: str) -> list[dict[str, Any]]:
    try:
        response = (
...
def eliminar_sesion(sesion_id: str, id_organizacion: str) -> bool:
    try:
        supabase.table("conversaciones").delete().eq("sesion_id", sesion_id).eq(
            "id_organizacion", id_organizacion
        ).execute()
        response = (
            supabase.table("sesiones_conversacion")
...
def actualizar_contexto(sesion_id: str, id_organizacion: str, contexto: dict[str, Any]) -> None:
    try:
        supabase.table("sesiones_conversacion").update({"contexto": contexto}).eq(
            "id", sesion_id
...
def guardar_turno(
    ...
    try:
        response = supabase.table("conversaciones").insert(payload).execute()
        data = response.data
        if data and isinstance(data, list) and len(data) > 0:
            return str(data[0].get("id", ""))
    except Exception as exc:
...
def construir_contexto_inicial(
    pregunta: str, respuesta: str, metadatos: dict[str, Any]
) -> dict[str, Any]:
    ...
    return {
        "palabras_clave": extraer_keywords(pregunta),
...
def actualizar_contexto_con_turno(
    ...
    nuevas = extraer_keywords(pregunta)
