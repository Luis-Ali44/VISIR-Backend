...
def expandir_abreviaturas(texto: str) -> str:
    ...
    texto_upper = re.sub(
        r"\bM\.?\s*N\.?\b|\bM/N\b|\bMN\b|\bPESOS?\b|\bPESO\s+MEXICANO\b", "MXN", texto_upper
    )
    texto_upper = re.sub(r"\bDLS?\b|\bDOLAR(?:ES)?\b", "USD", texto_upper)
    texto_upper = re.sub(
        r"PAGO\s+EN\s+UNA\s+SOLA\s+EXHIBICI[OÓ]N|UNA\s+SOLA\s+EXHIBICI[OÓ]N", "PUE", texto_upper
    )
    texto_upper = re.sub(
        r"PAGO\s+EN\s+PARCIALIDADES\s+O\s+DIFERIDO|PARCIALIDADES|DIFERIDO", "PPD", texto_upper
    )
    mapeo_formas = {
        r"\bEFECTIVO\b": "01",
...
CATALOGO_TIPO_RELACION: dict[str, str] = {
    ...
    "SUSTITUCIÓN": "04",
    "05": "05",
    "TRASLADOS DE MERCANCIAS": "05",
    "06": "06",
    "FACTURA POR TRASLADO": "06",
    "07": "07",
    "ANTICIPO APLICADO": "07",
    "ANTICIPO": "07",
    "08": "08",
    "FACTURA POR PAGOS": "08",
    "09": "09",
    "LEYENDA": "09",
}
