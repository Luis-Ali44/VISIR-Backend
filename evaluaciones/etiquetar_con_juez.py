...
def main() -> None:
    ...
    if output_path.exists():
        ...
        print(f"[REANUDANDO] {len(etiquetados)} filas ya procesadas, {len(dataset_features) - len(ids_procesados)} pendientes")
    ...
    for i, item in enumerate(dataset_features, 1):
        ...
        if item_id in ids_procesados:
            print(f"[{i}/{len(dataset_features)}] {item['pregunta'][:60]} -- ya procesado, saltando")
            continue
        ...
        try:
            ...
            if espera is not None:
                ...
                try:
                    ...
                    errores.append({"idx": i, "id": item_id, "pregunta": pregunta, "error": str(e2)})
                    continue
            else:
