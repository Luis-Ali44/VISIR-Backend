...
from evaluaciones.fixtures_org import CFDIS_SINTETICOS, FILENAMES_ESPERADOS, limpiar_fixtures
...
def indexar_fixtures(verbose: bool = True) -> None:
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"  INDEXANDO CFDIs SINTÉTICOS bajo UUID")
    ...
    if faltantes:
        raise RuntimeError(
            "Filenames esperados no indexados: " + ", ".join(sorted(faltantes))
        )
...
def stats() -> None:
    ...
    print(f"Org {ID_ORGANIZACION_PRUEBA}: {s['total_chunks']} chunks, {s['documentos_unicos']} documentos")
