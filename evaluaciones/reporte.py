...
def generar_reporte(
    ...
    if modo == "completo":
        scores_fidelidad = [
            r["fidelidad_score"] for r in resultados if r.get("fidelidad_score") is not None
        ]
        scores_relevancia = [
            r["relevancia_score"] for r in resultados if r.get("relevancia_score") is not None
        ]
