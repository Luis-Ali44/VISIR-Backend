...
def recuperar_para_evaluacion(
    ...
    if isinstance(retriever, OrgRAGRetriever):
        ...
        fragmentos = retriever.retrieve(
            query=pregunta, id_organizacion=id_organizacion, top_k=top_k, tipo_documento="cfdi"
        )
    else:
...
def ejecutar_evaluacion(args: argparse.Namespace) -> None:
    ...
    print(f"  top_k: {top_k}")
    print("  Matching: por nombre de archivo (fragmentos_fuente)")
    print(f"{'═' * 65}\n")
    ...
    for i, item in enumerate(dataset, 1):
        ...
        if modo == "completo" and juez is not None:
            if isinstance(retriever, OrgRAGRetriever):
                fragmentos_raw = retriever.retrieve(
                    query=pregunta, id_organizacion=org_id, top_k=top_k, tipo_documento="cfdi"
                )
            else:
