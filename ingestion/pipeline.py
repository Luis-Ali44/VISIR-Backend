...
class RAGIngestionPipeline:
    def __init__(self, config: RAGConfig = DEFAULT_CONFIG):
        ...
        self.topic_seed_profiles = self._load_topic_seed_profiles(Path(config.topic_seed_path))
        ...
        print(
            f"[INIT] ChromaDB '{stats['collection']}' — {stats['total_chunks']} chunks existentes"
        )
    ...
    def _load_topic_seed_profiles(self, seed_path: Path) -> list[TopicSeedProfile]:
        ...
        def flush_current() -> None:
            ...
            if not body:
                return
            profiles.append(
                TopicSeedProfile(title=current_title, text=body, priority=current_priority)
            )
    ...
    def _score_topic_importance(self, text: str) -> tuple[int, str, float, int]:
        ...
        doc_embedding = self.embed_model.get_text_embedding(self._compress_text_for_embedding(text))
    ...
    def ingest(
        ...
        sem_importance, topic_title, sem_sim, topic_priority = self._score_topic_importance(
            full_text
        )
        ...
        print(
            f"      ✅ new={batch_result['inserted']} | skip={batch_result['skipped']} | {elapsed}s"
        )
